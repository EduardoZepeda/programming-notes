...
if [ "$1" == "S" ]; then
    ...
    for folder in Cursos Udemy Nomadismo Libros; 
        ...
        if [ "$folder" = "Cursos" ]; then
            ...
            pandoc -f markdown -t epub3 -o output/epub/Cursos/ApuntesCursos.epub --metadata title="Apuntes Cursos" --epub-metadata=Cursos/metadata.xml --toc -s --toc-depth=1 --css=styles/pandoc_styles.css ${rstFiles[@]};   
        else
            for file in $folder/*.md; 
                do echo "Converting $file to epub"; 
                pandoc -f markdown -t epub -o output/epub/$file.epub  --toc --css=styles/pandoc_styles.css $file; 
                done; 
        fi
        done
